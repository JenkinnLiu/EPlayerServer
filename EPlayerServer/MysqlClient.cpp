...
int CMysqlClient::Connect(const KeyValue& args)
{
	...
	ret = mysql_real_connect(&m_db,
		...
		atoi(args.at("port")),
		NULL, 0);//mysql_real_connect连接数据库
...
{
	if (!m_bInit)return -1;
	int ret = mysql_real_query(&m_db, sql, sql.size());//mysql_real_query执行sql语句
	if (ret != 0) {
		printf("%s %s\n", __FUNCTION__, mysql_errno(&m_db));
		return -2;
	}
...
int CMysqlClient::Exec(const Buffer& sql, Result& result, const _Table_& table)
{
	...
	MYSQL_RES* res = mysql_store_result(&m_db); //mysql_store_result获取结果集
	MYSQL_ROW row;
	unsigned num_fields = mysql_num_fields(res);
	while ((row = mysql_fetch_row(res)) != NULL) {//mysql_fetch_row获取结果集中的一行
...
{
	if (!m_bInit)return -1;
	int ret = mysql_real_query(&m_db, "BEGIN", 6);//mysql_real_query执行sql语句,开始事务,
	if (ret != 0) {
		printf("%s %s\n", __FUNCTION__, mysql_errno(&m_db));
		return -2;
	}
...
{	//CREATE TABLE IF NOT EXISTS 表全名 (列定义,..., PRIMARY KEY `主键列名` ,UNIQUE INDEX `列名_UNIQUE` (列名 ASC) VISIBLE );
	...
	for (unsigned i = 0; i < FieldDefine.size(); i++)
	{
		...
		if (FieldDefine[i]->Attr & PRIMARY_KEY) {//主键
			sql += ",\r\n PRIMARY KEY (`" + FieldDefine[i]->Name + "`)";
		}
		if (FieldDefine[i]->Attr & UNIQUE) {//唯一
...
_mysql_field_::_mysql_field_(const _mysql_field_& field)//拷贝构造函数
{
	nType = field.nType;
	switch (field.nType)
	{
